...
def at_most_one_he(bool_vars, name):
    ...
    y = Bool(f"y_{name}")
    return And(And(at_most_one(bool_vars[:3] + [y])), And(at_most_one_he(bool_vars[3:] + [Not(y)], name+"_")))
...
def calculate_upper_bound(D, n):
    ...
    return sum(sorted_D[:n])

def solve_mcp(m, n, l, s, D, timeout = 300):
    iter = 0
    satisfiable = True
    best_value = 0
    best_solution = {}
    ...
    solver.add(max_distance<=calculate_upper_bound(D, (n//m)+1))
...
if __name__ == '__main__':
    filename = r"C:\Users\turek_hd7cu13\coding\gavno_gavna\Instances\inst07.dat"
    m, n, l, s, D = read_dat_file(filename)

    out = solve_mcp(m, n, l, s, D)
    model = out['model']
    x = out['x']

    print(f"Maximum distance: {out['distance']}")
    for i in range(m):
        tour = [j for k in range(n+2) for j in range(n+1) if model.eval(x[i][j][k])]
        print(f"Courier {i+1}: Tour {tour}")
    print(f"Time: {out['time']}")
    print(f"Optimal? : {out['optimal']}")