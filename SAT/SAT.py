...
if __name__ == '__main__':
    ...
    for index, inst in enumerate(instances[0:5]):
        filename = f"../instances/inst{inst}.dat"
        m, n, l, s, D = read_dat_file(filename)

        out = solve_mcp_sym(m, n, l, s, D)
        model = out['model']
        x = out['x']
        tours_sym = []
        ...
        out_nosym = solve_mcp_nosym(m, n, l, s, D)
        ...
        with open(f"../res/SAT/{names[index]}.json","w") as file:
